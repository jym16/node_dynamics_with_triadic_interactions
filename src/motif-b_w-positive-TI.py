...
def main():
    ...
    identifier = "motif-b_w-positive-TI"
    ...
    # """The external force."""
    # fex_amplitude = 1.0e0
    # fex_freq = np.pi * 1.0e2
    # external_force = lambda t: np.array([fex_amplitude, 0., 0.]).T * np.sin(fex_freq * t)
    ...
    cov_theo =  noise_std**2 / (2. * alpha * (alpha + w_neg) * (alpha + 3. * w_neg)) \
        * np.array([
