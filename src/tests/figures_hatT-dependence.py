...
def main():
    ...
    identifiers = ["Ra+", "C1a+", "C2a+", "C3a+"]
    _bins = 50 # 'fd' if you want to use Freedman-Diaconis rule

    output_file = "./figure/comparison_hatT-dependence.pdf"
    ...
    w_pos = 2.0
    w_neg = 1.0
    threshold = [1.0e-1, -1.0e-1, 1.0e-2, 1.0]
    alpha = 1.0e-1
    noise_std = 1.0e-1
    dt = 1.0e-2
    t_max = 1.0e2
    ...
    FIG_LABELS = ['(R)', '(C1)', '(C2)', '(C3)']
    RANGE = [(.9,.96), (.9,.96), (.9,.96), (.9,.96)]
    ...
    for i in range(len(identifiers)):
        ...
        axs[positions[i]].set_ylim(RANGE[i])
